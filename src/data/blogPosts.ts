...
export const BLOG_POSTS: BlogPost[] = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: "ul", items: [
        ...
        "Fraîcheur — des avis récents (__moins de 3 mois__) pèsent plus que des avis anciens dans le calcul de notoriété.",
  ...
  },
  {
    ...
    blocks: [
      { type: "p", text: "La question revient à chaque commerçant qui découvre un outil de réponse automatique : Google va-t-il sanctionner l'usage de l'IA sur sa fiche ? La réponse, d'après les règles publiques de Google Business Profile et les retours observés sur l'écosystème, est non — __sous conditions précises__." },
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: "p", text: "On monte le dossier à votre place — argumentaire, preuves, dépôt et suivi jusqu'à la décision de Google — pour **19,90€ par avis retiré**. __Satisfait ou remboursé__ : si Google refuse le retrait, vous êtes intégralement remboursé, sans démarche supplémentaire de votre part." },
    ],
  },
];
...
// Identité visuelle par catégorie (bandeau + pastille) — couleurs Google
// existantes du site, pas de nouvelle palette à maintenir.
export const CATEGORY_STYLE: Record<string, { icon: string; color: string; bg: string }> = {
  "Réputation": { icon: "💬", color: "#EA4335", bg: "linear-gradient(135deg, #EA4335, #C5221F)" },
  "Google Business Profile": { icon: "📍", color: "#1A73E8", bg: "linear-gradient(135deg, #1A73E8, #174EA6)" },
  "SEO local": { icon: "🔍", color: "#34A853", bg: "linear-gradient(135deg, #34A853, #1E7A3D)" },
  "IA & conformité": { icon: "🤖", color: "#7C3AED", bg: "linear-gradient(135deg, #7C3AED, #5B21B6)" },
  "Collecte d'avis": { icon: "📲", color: "#FBBC04", bg: "linear-gradient(135deg, #FBBC04, #C88A00)" },
};
