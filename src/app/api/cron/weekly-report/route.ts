    ...
    const { error: erreurResend } = await resend().emails.send({
      ...
      html: `
        <div style="font-family:'Inter',system-ui,sans-serif;max-width:560px;margin:0 auto;background:#fff;">
          <!-- Header -->
          <div style="background:#2457C5;padding:28px 32px;border-radius:16px 16px 0 0;">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
              ${["rgba(255,255,255,0.95)","rgba(255,255,255,0.72)","rgba(255,255,255,0.5)"].map(c => `<div style="width:8px;height:8px;border-radius:50%;background:${c};display:inline-block;"></div>`).join("")}
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:0;border:1px solid #DADCE0;border-top:none;">
            ${[
              { label: "Avis reçus", value: String(total), color: "#2457C5" },
              { label: "Positifs 4-5★", value: String(positive), color: "#16856B" },
              { label: "Négatifs 1-3★", value: String(negative), color: "#D6455D" },
              { label: "Note moyenne", value: avgRating === "–" ? "–" : `${avgRating}★`, color: "#E0A11A" },
            ].map((s, i) => `
          ...
          <div style="padding:24px 28px;">
            ${unanswered.length > 0 ? `
              <div style="background:#FFF3E0;border:1px solid #E0A11A;border-radius:10px;padding:16px 18px;margin-bottom:20px;">
            ...
            ` : `
              <div style="background:#E6F4EA;border:1px solid #16856B;border-radius:10px;padding:16px 18px;margin-bottom:20px;">
              ...
              ${weeklyReviews.slice(0, 5).map(r => `
                <div style="border:1px solid #DADCE0;border-radius:10px;padding:14px 16px;margin-bottom:8px;">
                  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                    <span style="font-size:14px;color:#E0A11A;">${stars(r.review.rating)}</span>
            ...
            <a href="${dashboardUrl}/dashboard" style="display:block;text-align:center;margin-top:24px;padding:14px 24px;background:#2457C5;color:#fff;border-radius:10px;text-decoration:none;font-weight:700;font-size:15px;">
          ...
          <div style="padding:16px 28px;border-top:1px solid #DADCE0;font-size:11px;color:#80868B;text-align:center;">
            Caela Réputation by Caela Agency · <a href="mailto:contact@caela.fr" style="color:#2457C5;text-decoration:none;">contact@caela.fr</a>
